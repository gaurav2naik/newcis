...
<%
	...
	if (from != null && to != null && from != "" && to != "") {
        ...
        findobj1.append("StartTime", new BasicDBObject("$gt",from_timestamp+19800000).append("$lt", to_timestamp+19800000+43200000));
...
$(document).ready(function() {
	...
	$("#0").click(function() {
		...
		$("#LINKS").load("premiumgraphs/onlylink.jsp?DEVICE="+"<%=Device%>"+"&VALUE="+"<%=arr[0]%>");	
	});

		$("#1").click(function() {
			$("#LINKS").empty();
			$('.cssload-container').show();
			$("#LINKS").load("premiumgraphs/onlylink.jsp?DEVICE="+"<%=Device%>"+"&VALUE="+"<%=arr[1]%>");
		});
		$("#2").click(function() {
			$("#LINKS").empty();
			$('.cssload-container').show();
			$("#LINKS").load("premiumgraphs/onlylink.jsp?DEVICE="+"<%=Device%>"+"&VALUE="+"<%=arr[2]%>");
		});

		$("#3").click(function() {
			$("#LINKS").empty();
			$('.cssload-container').show();
			$("#LINKS").load("premiumgraphs/onlylink.jsp?DEVICE="+"<%=Device%>"+"&VALUE="+"<%=arr[3]%>");
		});
		$("#4").click(function() {
			$("#LINKS").empty();
			$('.cssload-container').show();
			$("#LINKS").load("premiumgraphs/onlylink.jsp?DEVICE="+"<%=Device%>"+"&VALUE="+"<%=arr[4]%>");
						});
...
<body onload="showDiv()">
	...
	if(alertData1.size()!=0){ %>
		<div id="digitaljourney"
			style="height: 46%; width: 22%; background-color: white; color: black; margin-left: 1%; float: left; margin-top: 0%;">
				...
				%><font size="2"><p>
						...
						<%
							System.out.println(arr[i]);
									String path = GetBandwidthPremiumUser.SelectPicturePath(Device, arr[i]);
									System.out.println(path);
						%>
					</font>
				<button id=<%=i%>>view</button>
				<img src=<%=path%> style="width: 6%; padding-top: 1.2%" align="top">
				</p>
				<%
					}
					}
 ...
 %>
		...
		<%} %>
		<div id="daterange" >
		<form action="Hotuser.jsp?Device=<%=Device %>" method="post" onsubmit="return CheckDateValue()">
		...
		<div id="LINKS" style="padding-top: 2%">
			...
			<jsp:include page="premiumgraphs/onlylink.jsp">
				<jsp:param name="DEVICE" value="<%=Device%>" />
				<jsp:param name="VALUE" value="<%=firstvalue[0]%>" />
			</jsp:include>
		</div>
		<%} else{%>
